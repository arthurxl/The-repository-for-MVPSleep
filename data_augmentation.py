...
def gaussian_noise(data, mean=0, sigma=0.1):

    noise = np.random.normal(mean, sigma, data.shape).astype(dtype=np.float32)
    output = data + noise
    return output


def time_reverse(x):
    l = np.random.randint(1, 2999)
    x[:, :l], x[:, l:] = x[:, -l:], x[:, :-l]
    return x


def sign_flip(x):
    l = np.random.randint(1, 2000)
    x[:, l:max(l + 1000, 3000)] = -x[:, l:max(l + 1000, 3000)]
    return x


class AugmentationTWO:
    ...
    def __call__(self, x):
        x = self.pre_norm(x)
        data = self.train_transform(x).view(1, 129, 29)
        return data, data


class MixUp(nn.Module):
    def __init__(self, args, ratio=0.4):
        super().__init__()
        self.args = args
        self.ratio = ratio
    def forward(self, x):
        xa = x
        xb = torch.index_select(xa, 0, torch.randperm(x.size(0)).to(x.get_device()))
        x = self.ratio * xa + (1. - self.ratio) * xb
        return x
...
class RandomResizeCrop(nn.Module):
    ...
    def forward(self, lms):
        ...
        virtual_crop_area = (torch.zeros((lms.shape[1], virtual_crop_size[0], virtual_crop_size[1]))
...
class RunningNorm(nn.Module):
    ...
    def __init__(self, epoch_samples, max_update_epochs=10, axis=1):
        super().__init__()
